...
public class MushroomTower : MonoBehaviour
{
    ...
    public GameObject mushShotPrefab;
    ...
    void Shoot()
    {
        GameObject mushballGO = (GameObject)Instantiate (mushShotPrefab, firePoint.position, firePoint.rotation); //casting to game oject
        MushShot mushball = mushballGO.GetComponent<MushShot>();

        if (mushball != null) 
        {
            mushball.Seek(target);
        }
    }
