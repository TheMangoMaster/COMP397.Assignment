...
public class CornTower : MonoBehaviour
{
    ...
    public float fireCountdown = 0f; //how quickly we can shoot again
    ...
    public GameObject cornshotPrefab;
    ...
    void Shoot()
    {        
        GameObject cornshotGO = (GameObject)Instantiate(cornshotPrefab, Random.insideUnitSphere * 0.5f + firePoint.position, firePoint.rotation); //casting to game oject
        CornShot cornshot = cornshotGO.GetComponent<CornShot>();

        if (cornshot != null)
        {
            cornshot.Seek(target);
        }      
